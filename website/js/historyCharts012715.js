...
HistoryTable.Initialize = function(){
	...
	HistoryTable.visibility[4] = true;
        ...
        tooltip: {
            formatter: function () {
            	...
            	if (this.point.hasOwnProperty("numShortTrades"))
					retVal += this.point.numShortTrades + "BD, ";
//            	if (this.point.hasOwnProperty("numReversalTrades"))
//					retVal += this.point.numReversalTrades + "r, "; 
            	...
            	if (this.point.hasOwnProperty("numBreakdownTrades"))
					retVal += this.point.numBreakdownTrades + "sl"; 
				retVal += ")</p>";
            	return retVal;
            }
				...
				events: { 
					legendItemClick:function(e) {
						...
						if (seriesIndex != CONSTS.TOTALS) {
							...
							HistoryTable.Chart.toggleStrategy(CONSTS.BREAKOUTS, true);
							HistoryTable.Chart.toggleStrategy(CONSTS.BREAKDOWNS, true);
...
HistoryTable.Chart.toggleStrategy = function(strategyIndex, makeVisible, saveVisibility){
	...
	switch (strategyIndex){
		...
		case CONSTS.SHORTS:
			rowClass = "backdraft";
    		break;
		case CONSTS.BREAKOUTS:
			rowClass = "breakout";
    		break;
		case CONSTS.BREAKDOWNS:
			rowClass = "breakdown";
    		break;
		case CONSTS.TOTALS:
			rowClass = "total";
...
HistoryTable.List.ChangeTotals = function(monthStr, seriesIndex){
	...
	var $shortsTotal = $("div#BackdraftSummary");
	var $breakoutsTotal = $("div#BreakoutSummary");
	var $breakdownsTotal = $("div#BreakdownSummary");
	...
	if (HistoryTable.visibility[CONSTS.PULLBACKS] && (seriesIndex == -1 || seriesIndex == CONSTS.PULLBACKS)){
		...
		$pullbacksTotal.find(".summaryData").text(monthTotals["2"]).formatCurrency({colorize:true});
		total += monthTotals["2"];
		showCount ++;
	} else {
	...
	if (HistoryTable.visibility[CONSTS.SHORTS] && (seriesIndex == -1 || seriesIndex == CONSTS.SHORTS)){
		...
		$shortsTotal.find(".summaryData").text(monthTotals["4"]).formatCurrency({colorize:true});
		total += monthTotals["4"];
		showCount ++;
	} else {
	...
	if (HistoryTable.visibility[CONSTS.BREAKOUTS] && (seriesIndex == -1 || seriesIndex == CONSTS.BREAKOUTS)){
		$breakoutsTotal.removeClass("none");
		$breakoutsTotal.find(".summaryData").text(monthTotals["1"]).formatCurrency({colorize:true});
		total += monthTotals["1"];
		showCount ++;
	} else {
		$breakoutsTotal.addClass("none");
	} 	
	
	if (HistoryTable.visibility[CONSTS.BREAKDOWNS] && (seriesIndex == -1 || seriesIndex == CONSTS.BREAKDOWNS)){
		$breakdownsTotal.removeClass("none");
		$breakdownsTotal.find(".summaryData").text(monthTotals["3"]).formatCurrency({colorize:true});
		total += monthTotals["3"];
		showCount ++;
	} else {
		$breakdownsTotal.addClass("none");
	} 	

...
*/
    static function retrieveCurrentPrice($stock, $requestFields) {
        $url = BASE_URL_;
        if (is_array($stock)) {
            ...
            echo $url = $url.$stockquery.'&f='.$requestFields.STATIC_END_ . "</br>";
        } else {
//                die('YAHOO FINANCE RETRIEVECURRENTPRICE IS NOT EQUIPPED TO HANDLE NONARRAY PARAMETER');
            $url = BASE_URL_.$stock.'&f='.$requestFields.STATIC_END_. "</br>";
        }
        if(!$file=fopen($url,'r'))
